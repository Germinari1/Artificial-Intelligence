...
def minimax(board, alpha=-float('inf'), beta=float('inf')):
  ...
  if player(board) == X:
    value, move = max_value(board, alpha, beta)
  else:
    value, move = min_value(board, alpha, beta)
...
def max_value(board, alpha, beta):
  ...
  for action in actions(board):
    aux, act = min_value(result(board, action), alpha, beta)
    if aux > v:
      v = aux
      move = action
...
def min_value(board, alpha, beta):
  ...
  move = None
  for action in actions(board):
    aux, act = max_value(result(board, action), alpha, beta)
    if aux < v:
      v = aux
      move = action
